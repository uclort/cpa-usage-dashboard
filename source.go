...
func fetchSource(ctx context.Context, host hostClient, cfg pluginConfig, source usageSource) overviewSource {
	out := overviewSource{ID: source.identity(), Provider: source.Provider, Name: firstNonEmpty(source.Name, source.ID), Status: "error", Items: []usageItem{}}
	if source.URL == "" || source.FieldTotal == "" || source.FieldUsed == "" {
		out.Error = &usageItemError{Code: "invalid_source", Message: "url、field_total、field_used 都是必填"}
		return out
	}
